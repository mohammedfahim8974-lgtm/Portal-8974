...
content = content.replace(/recordsForDay\.forEach\(record => \{\s*if \(record\.status !== 'absent'\) \{\s*isAbsent = false;\s*sitesCount\+\+;\s*const h = Number\(record\.hours\) \|\| 0;\s*const ot = Number\(record\.otHours\) \|\| 0;\s*const calcOt = ot > 0 \|\| \(h <= standardHours\) \? ot : Math\.max\(0, h - standardHours\);\s*const calcNorm = h > 0 \? \(h > standardHours \? standardHours : h\) : 0;\s*dayNorm \+= calcNorm;\s*dayOT \+= calcOt;\s*\}\s*\}\);/g,
...
console.log("Done updating LabourCard.tsx second loop");
