...
export const computeSiteReport = (
  ...
  deduplicateAttendanceRecords(attendance).forEach((record) => {
      ...
      if (record.workerIds) {
        ...
        record.workerIds.forEach((workerId) => {
          ...
          if (dateObj.getDay() === 0) { // Sunday
            dayWorkerCost = (h * 1.5 + 9) * hourlyRate;
          } else {
            // normal day
...
export const ConstructionCalculator: React.FC<ConstructionCalculatorProps> = ({
  ...
  const handleSyncFromSite = () => {
    ...
    workers.forEach((worker) => {
      ...
      } else if (isAssigned && worker.status === "Active") {
  ...
  React.useEffect(() => {
    if (selectedSite && selectedSite !== "ALL_SITES") {
      handleSyncFromSite();
    } else if (selectedSite === "ALL_SITES") {
      setEntries([]);
      setAbsentWorkers([]);
    }
  }, [selectedSite, localSelectedMonth, calculationPeriod, attendance, workers]);
